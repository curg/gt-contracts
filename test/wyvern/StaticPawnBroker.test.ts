...
import { atomicMatchArgs, Call, HowToCall, Order, orderTypes, staticCallArgs } from "./lib";

describe("StaticPawnBroker", function () {
  ...
  describe("static functions", function () {
    it("should accept valid pair", async function () {
      const { bob, statici, order1, order2, call1, call2 } = await loadFixture(fixture);

      await expect(statici.ERC721PawnForERC20(...staticCallArgs(order1, call1, order2, call2, bob.address, 0, 0))).not
        .to.be.reverted;
      await expect(statici.ERC20ForERC721Pawn(...staticCallArgs(order2, call2, order1, call1, bob.address, 0, 0))).not
        .to.be.reverted;
    });

    it("should reject invalid pair", async function () {
      const { bob, statici, order1, order2, call1 } = await loadFixture(fixture);

      await expect(statici.ERC721PawnForERC20(...staticCallArgs(order1, call1, order2, call1, bob.address, 0, 0))).to.be
        .reverted;
      await expect(statici.ERC20ForERC721Pawn(...staticCallArgs(order2, call1, order1, call1, bob.address, 0, 0))).to.be
        .reverted;
    });
  });
